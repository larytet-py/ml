...
python3 backtest_weekly_option_reversal_weekend.py \
  --symbol SPY \
  --side put \
  --optimize
"""
...
def compute_weekend_entry_candidates(
    ...
    friday_mask = date_series.dt.weekday.to_numpy(dtype=int, copy=False) == 4
    ...
    for i, is_friday in enumerate(friday_mask):
        if not is_friday:
            continue
        if not allow_overlap and i < next_entry_idx:
            continue
        if pd.isna(roc[i]) or pd.isna(trend[i]) or pd.isna(pricing_vol[i]):
            continue
        if side == "put":
            if not (roc[i] <= put_roc_threshold and trend[i] >= downside_vol_threshold_annualized):
                continue
        else:
            if not (roc[i] >= call_roc_threshold and trend[i] >= upside_vol_threshold_annualized):
                continue

        entry_date = pd.Timestamp(date_values[i]).normalize()
        scheduled_expiry_date = entry_date + pd.Timedelta(days=7)
        exit_idx = friday_index_by_date.get(scheduled_expiry_date)
        if exit_idx is None or exit_idx <= i:
            continue
...
def run_backtest(
    ...
    entries = compute_weekend_entry_candidates(
        signal_df=signal_df,
        side=side,
    ...
    for i, entry in entries.items():
        ...
        scheduled_expiry_date = (entry_date + pd.Timedelta(days=7)).normalize()
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Backtest: sell ATM puts/calls only at Friday close after a ROC + directional stddev trigger, "
            "then settle at the next Friday close."
        )
    )
    ...
    parser.add_argument("--allow-overlap", action="store_true", help="Allow overlapping Friday-to-Friday positions.")
    ...
    if args.optimize:
        ...
        if not result.params:
            ...
            parameter_lines.append("expiry=next-friday")
