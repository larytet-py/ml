...
def run_backtest(
    ...
    for i in range(len(df)):
        ...
        row = df.iloc[i]
        if pd.isna(row["roc"]) or pd.isna(row["downside_vol_annualized"]) or pd.isna(row["pricing_vol_annualized"]):
            continue

        trigger = (
            row["roc"] <= roc_threshold
        ...
        intrinsic = max(strike - exit_close, 0.0)
        ...
        trades.append(
            Trade(
                ...
                downside_vol_signal=float(row["downside_vol_annualized"]),
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Backtest: sell ATM weekly put after downside shock (ROC + downside stddev trigger)."
    )
    ...
    parser.add_argument("--roc-threshold", type=float, default=-0.03, help="Trigger when ROC <= threshold.")
    parser.add_argument("--vol-window", type=int, default=20, help="Rolling window for downside stddev.")
    parser.add_argument(
        "--downside-vol-threshold",
        type=float,
        default=0.20,
        help="Annualized downside vol threshold for signal.",
    )
